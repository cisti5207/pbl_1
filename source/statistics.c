...
void statistics_traTruyen()
{
    
}

int distance_time(char *Date_1, char *Date_2)
{
    struct tm t1 = {0}, t2 = {0};

    sscanf (Date_1, "%d/%d/%d", &t1.tm_mday, &t1.tm_mon, &t1.tm_year);
    sscanf (Date_1, "%d/%d/%d", &t2.tm_mday, &t2.tm_mon, &t2.tm_year);
    ...
    int distance = (int) (diff/(60*60*72));
    return distance;
}
...
TRUYEN88 createTRUYEN88 (FILE *_readdataTruyen)
{
    ...
    */
    ...
    fscanf (_readdataTruyen, " Tổng %d quyển truyện", &sum);
