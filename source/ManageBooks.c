...
void InitManageBooks(Role _role)
{
    ...
    while (!WindowShouldClose())
    {
        ...
        if (State == MANAGEBOOKS_Add || State == MANAGEBOOKS_Detail || isFiltering)
        {
            ...
            Rectangle btnBackHeader = { customUI.HeaderBox.x + marginX, customUI.HeaderBox.y + (customUI.HeaderBox.height - btnHeight) / 2.0f, btnWidthBack, btnHeight };
            ...
            DrawTextEx(_Font[0], textBack, (Vector2){btnBackHeader.x + (btnBackHeader.width - sizeBack.x) / 2.0f, btnBackHeader.y + (btnBackHeader.height - sizeBack.y) / 2.0f}, fontSizeBtn, 1, WHITE);
        ...
        if (isShowingBookList)
        {
            ...
            Rectangle filterBox = { customUI.HeaderBox.x + (customUI.HeaderBox.width - filterW) / 2.0f, customUI.HeaderBox.y + (customUI.HeaderBox.height - filterH) / 2.0f, filterW, filterH };
            ...
            DrawRectangleRounded(filterBox, 1.0f, 20, Fade(DARKGRAY, 0.7f));
            DrawRectangleRoundedLinesEx(filterBox, 1.0f, 20, 2.0f, Fade(LIGHTGRAY, 0.3f));
            ...
            DrawRectangleRounded((Rectangle){handleRec.x, handleRec.y + 2, handleRec.width, handleRec.height}, 1.0f, 20, Fade(BLACK, 0.3f));
            DrawRectangleRounded(handleRec, 1.0f, 20, TEALBLUE);
            DrawRectangleRoundedLinesEx(handleRec, 1.0f, 20, 1.0f, Fade(WHITE, 0.4f));

            int fSize = fontSizeBtn * 0.8f;
            Color colorAll = (animRatio < 0.5f) ? WHITE : Fade(LIGHTGRAY, 0.6f);
            Color colorInStock = (animRatio > 0.5f) ? WHITE : Fade(LIGHTGRAY, 0.6f);

            DrawTextEx(_Font[3], "Tất cả", (Vector2){btnAll.x + (btnAll.width - MeasureTextEx(_Font[3], "Tất cả", fSize, 1).x)/2, btnAll.y + (btnAll.height - fSize)/2}, fSize, 1, colorAll);
            DrawTextEx(_Font[3], "Còn kho", (Vector2){btnInStock.x + (btnInStock.width - MeasureTextEx(_Font[3], "Còn kho", fSize, 1).x)/2, btnInStock.y + (btnInStock.height - fSize)/2}, fSize, 1, colorInStock);
        }
        ...
        if (_role == ADMINISTRATOR && State != MANAGEBOOKS_Add)
        {
            if (State != MANAGEBOOKS_Detail) 
            {
                ...
                Rectangle btnAddHeader = { customUI.HeaderBox.x + customUI.HeaderBox.width - btnWidthAdd - marginX, customUI.HeaderBox.y + (customUI.HeaderBox.height - btnHeight) / 2.0f, btnWidthAdd, btnHeight };
                ...
                DrawTextEx(_Font[3], textAdd, (Vector2){btnAddHeader.x + (btnAddHeader.width - sizeAdd.x) / 2.0f, btnAddHeader.y + (btnAddHeader.height - sizeAdd.y) / 2.0f}, fontSizeBtn, 1, WHITE);
            ...
            else 
            {
                ...
                Rectangle btnEditHeader = { customUI.HeaderBox.x + customUI.HeaderBox.width - btnWidthEdit - marginX, customUI.HeaderBox.y + (customUI.HeaderBox.height - btnHeight) / 2.0f, btnWidthEdit, btnHeight };
                bool hoverEdit = CheckCollisionPointRec(ManageBooksSize.Mouse, btnEditHeader);
                DrawRectangleRounded(btnEditHeader, 0.2f, 10, hoverEdit ? Fade(GOLD, 0.8f) : GOLD);
                
                const char* textEdit = "Chỉnh Sửa";
                Vector2 sizeEdit = MeasureTextEx(_Font[3], textEdit, fontSizeBtn, 1);
                DrawTextEx(_Font[3], textEdit, (Vector2){btnEditHeader.x + (btnEditHeader.width - sizeEdit.x) / 2.0f, btnEditHeader.y + (btnEditHeader.height - sizeEdit.y) / 2.0f}, fontSizeBtn, 1, BLACK);
...
int ShowBookDetail_Panel(Size size, ManageBooksUI UI, float *wheel, Font *_Font, BookList *Books, int bookIndex, MANAGEBOOKS_STATE *State, MANAGEBOOKS_STATE prevState, Role _role)
{
    ...
    Color bgColor = (Color){ 24, 25, 26, 255 }; 
    ...
    DrawTextEx(_Font[0], book.TypeBook, (Vector2){ rightX + 220, currentY }, fontSizeInfo, 1, (Color){ 255, 165, 0, 255 });
    currentY += spacingY;
    ...
    DrawTextEx(_Font[0], buffer, (Vector2){ rightX + 220, currentY }, fontSizeInfo, 1, book.TotalStock > 0 ? GREEN : RED);
    currentY += spacingY;
    ...
    DrawTextAutoWrap(_Font[0], (book.Description[0] != '\0') ? book.Description : "Chưa có giới thiệu...", descHitbox, 18.0f, 1.0f, Fade(LIGHTGRAY, 0.8f));

    DrawTextEx(_Font[0], "DANH SÁCH TẬP", (Vector2){ leftX, listY }, 24, 1, TEALBLUE);
    DrawLineEx((Vector2){leftX, listY + 35}, (Vector2){detailArea.x + detailArea.width - 50, listY + 35}, 2.0f, Fade(TEALBLUE, 0.5f));
    ...
    if (displayCount == 0)
    {
        DrawTextEx(_Font[0], "Truyện này chưa có tập nào.", (Vector2){ listArea.x + 20, listArea.y + 20 }, 20, 1, GRAY);
        return signalReturn;
    }
    ...
    for (int j = 0; j < displayCount; j++)
    {
        ...
        Color itemBg = isHoverItem ? (Color){ 40, 45, 50, 255 } : (Color){ 30, 32, 35, 255 };
        DrawRectangleRounded(itemRec, 0.2f, 10, itemBg);

        float numBoxSize = itemHeight * 0.6f;
        Rectangle numBox = { itemRec.x + 15, itemRec.y + (itemHeight - numBoxSize)/2, numBoxSize * 1.3f, numBoxSize };
        DrawRectangleRounded(numBox, 0.3f, 10, Fade(TEALBLUE, 0.2f));
        DrawRectangleRoundedLinesEx(numBox, 0.3f, 10, 1.5f, Fade(TEALBLUE, 0.6f));
        
        char numStr[16];
        snprintf(numStr, sizeof(numStr), "%d", j + 1);
        float numFont = numBoxSize * 0.6f;
        float nw = MeasureTextEx(_Font[0], numStr, numFont, 1).x;
        DrawTextEx(_Font[0], numStr, (Vector2){ numBox.x + (numBox.width - nw)/2, numBox.y + (numBox.height - numFont)/2 }, numFont, 1, LIGHTGRAY);

        float volNameFont = itemHeight * 0.4f;
        DrawTextEx(_Font[3], book.volumes[j].VolumeName, (Vector2){ numBox.x + numBox.width + 15, itemRec.y + (itemHeight - volNameFont)/2 }, volNameFont, 1, isHoverItem ? TEALBLUE : LIGHTGRAY);
        ...
        if (_role == ADMINISTRATOR)
        {
            ...
            DrawRectangleRounded(btnDel, 0.25f, 10, hoverDel ? MAROON : Fade(ERRORRED, 0.85f));
        ...
        DrawLineEx((Vector2){ itemRec.x + 10, itemRec.y + itemHeight + itemSpacing / 2 }, (Vector2){ itemRec.x + itemRec.width - 10, itemRec.y + itemHeight + itemSpacing / 2 }, 1.0f, Fade(DARKGRAY, 0.3f));
    }
    ...
    if (TotalContentHeight > listArea.height)
    {
        ...
        if (isDraggingDetailVol)
        {
            ...
            {
                float newHandleY = size.Mouse.y - dragOffsetDetailVolY;
                if (newHandleY < sTrack.y)
                    newHandleY = sTrack.y;
                if (newHandleY > sTrack.y + sTrack.height - sHandleH)
                    newHandleY = sTrack.y + sTrack.height - sHandleH;
                float newRatio = (newHandleY - sTrack.y) / (sTrack.height - sHandleH);
                *wheel = -(newRatio * (TotalContentHeight - listArea.height));
                sHandle.y = newHandleY;
            }
            else
            {
        ...
        DrawRectangleRounded(sHandle, 1.0f, 10, isHoveredScroll ? GRAY : Fade(DARKGRAY, 0.8f));
    }
...
int ShowAddBook_Panel(Size size, ManageBooksUI UI, Font *_Font, InputBox *inputs, bool isEditMode, Book *editBook, float *volWheel)
{
    ...
    if (itemHeight < 40.0f) itemHeight = 40.0f;
    if (itemHeight > 60.0f) itemHeight = 60.0f;
    float itemSpacing = itemHeight * 0.2f;
    ...
    float totalContentHeight = abs_btnSaveY + btnHeight + 40.0f;
    ...
    if (panelArea.y + masterScrollY + panelArea.height * 0.03f > panelArea.y)
    {
        ...
        DrawTextEx(_Font[0], titleText, (Vector2){panelArea.x + panelArea.width * 0.05f, panelArea.y + masterScrollY + panelArea.height * 0.03f}, titleFontSize, 1, GOLDACCENT);
        DrawLineEx((Vector2){panelArea.x + panelArea.width * 0.05f, panelArea.y + masterScrollY + panelArea.height * 0.09f}, 
                   (Vector2){panelArea.x + panelArea.width * 0.95f, panelArea.y + masterScrollY + panelArea.height * 0.09f}, 3, GRAY);
    }
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        DrawTextEx(_Font[0], (i == 7) ? "Mô Tả Truyện:" : labels[i], (Vector2){xPos, yPos}, (i==7) ? labelFontSize + 2.0f : labelFontSize, 1, (i==7) ? ORANGE : LIGHTGRAY);
        ...
        if (displayError)
        {
            ...
            DrawTextEx(_Font[0], errMsg, (Vector2){xPos + errBoxW - errW, yPos + (labelFontSize - labelFontSize*0.8f)}, labelFontSize * 0.8f, 1, RED);
        }

        float roundness = (i == 7) ? 0.05f : 0.2f; 
        Color boxBgColor = (i == 0) ? Fade(DARKGRAY, 0.4f) : ((inputs[i].isFocused || hasTextArr[i]) ? WHITESMOKE : GRAY);
        Color boxOutlineColor = (i == 0) ? GRAY : (displayError ? RED : (inputs[i].isFocused ? TEALBLUE : DARKGRAY));
        float outlineThick = (i == 0) ? 2.0f : (inputs[i].isFocused ? 3.0f : (displayError ? 2.5f : 2.0f));
        ...
        if (clipArea.width > 0 && clipArea.height > 0)
        {
            ...
            if (i < 7)
            {
                ...
                DrawTextEx(_Font[3], inputs[i].text, (Vector2){textX, textY}, inputTextSize, 1, (i == 0) ? ORANGE : BLACK);
            ...
            else
            {
                ...
                if (inputs[i].isFocused && !showAddVolPopup && ((int)(GetTime() * 2) % 2 == 0))
                    strcat(tempText, "|");
                DrawTextAutoWrap(_Font[3], tempText, textArea, inputTextSize, 1.0f, BLACK);
            }
    ...
    {
        DrawTextEx(_Font[0], "Tập truyện:", (Vector2){offsetX, volLabelY}, labelFontSize + 2.0f, 1, ORANGE);
        ...
        Color btnAddVolColor = isBasicInfoFilled ? (hoverAddVol ? MAROON : BLUE) : Fade(GRAY, 0.6f);
        
        DrawRectangleRounded(btnAddVol, 0.2f, 10, btnAddVolColor);
        float addVolFont = labelFontSize * 0.9f;
        DrawTextEx(_Font[3], "+ Thêm tập", (Vector2){ btnAddVol.x + (130 - MeasureTextEx(_Font[3], "+ Thêm tập", addVolFont, 1).x)/2, btnAddVol.y + (btnAddVol.height - addVolFont)/2 }, addVolFont, 1, isBasicInfoFilled ? WHITE : DARKGRAY);
    ...
    float listStartY = panelArea.y + masterScrollY + abs_listStartY;
    Rectangle listArea = { offsetX, listStartY, contentWidth, fixedListHeight };
    static int activeVolIndex = -1;
    static char volInputText[16] = "";

    if (listArea.y + listArea.height >= panelArea.y && listArea.y <= panelArea.y + panelArea.height)
    {
        DrawRectangleRec(listArea, (Color){ 20, 22, 25, 255 });
        DrawRectangleLinesEx(listArea, 1.0f, DARKGRAY);

        Rectangle clipListArea = GetCollisionRec(listArea, panelArea);
        if (clipListArea.width > 0 && clipListArea.height > 0)
        {
            ...
            if (editBook->volumeCount > 0)
            {
                ...
                {
                    float yPos = listArea.y + localVolScroll + j * (itemHeight + itemSpacing);
                    
                    if (yPos + itemHeight < clipListArea.y || yPos > clipListArea.y + clipListArea.height)
                        continue; 

                    Rectangle itemRec = { listArea.x + 10, yPos + itemSpacing/2, listArea.width - 20, itemHeight };
                    DrawRectangleRounded(itemRec, 0.2f, 10, (Color){ 40, 45, 50, 255 });

                    float volNameFont = itemHeight * 0.45f;
                    DrawTextEx(_Font[3], editBook->volumes[j].VolumeName, (Vector2){ itemRec.x + 15, itemRec.y + (itemHeight - volNameFont)/2 }, volNameFont, 1, LIGHTGRAY);
                    ...
                    DrawRectangleRounded(btnPlus, 0.2f, 5, hoverPlus ? LIME : TEALBLUE);
                    ...
                    if (activeVolIndex == j && !showAddVolPopup)
                    {
                        ...
                        DrawRectangleRounded(stockBox, 0.2f, 5, WHITESMOKE);
                        DrawRectangleRoundedLinesEx(stockBox, 0.2f, 5, 2.0f, TEALBLUE);
                        float textFont = btnSize * 0.7f;
                        float textW = MeasureTextEx(_Font[0], volInputText, textFont, 1).x;
                        DrawTextEx(_Font[0], volInputText, (Vector2){ stockBox.x + (stockBoxW - textW)/2, stockBox.y + (btnSize - textFont)/2 }, textFont, 1, BLACK);
                        
                        if ((int)(GetTime()*2)%2==0)
                            DrawRectangle(stockBox.x + (stockBoxW - textW)/2 + textW + 2, stockBox.y + btnSize*0.1f, 2, btnSize*0.8f, BLACK);
                    }
                    else
                    {
                        DrawRectangleRounded(stockBox, 0.2f, 5, Fade(DARKGRAY, 0.4f));
                        char stockStr[32];
                        ...
                        DrawTextEx(_Font[0], stockStr, (Vector2){ stockBox.x + (stockBoxW - textW)/2, stockBox.y + (btnSize - textFont)/2 }, textFont, 1, editBook->volumes[j].ImportStock > 0 ? GREEN : RED);
                    }
                    ...
                    DrawRectangleRounded(btnMinus, 0.2f, 5, hoverMinus ? MAROON : ERRORRED);
            ...
            {
                DrawTextEx(_Font[0], "Truyện này chưa có tập nào.", (Vector2){ listArea.x + 15, listArea.y + 15 }, 20, 1, GRAY);
                activeVolIndex = -1;
            }
            ...
            if (volContentHeight > fixedListHeight)
            {
                ...
                Rectangle vTrack = { listArea.x + listArea.width - vTrackWidth - 2, listArea.y, vTrackWidth, listArea.height };
                ...
                if (clipTrack.height > 0)
                {
                    ...
                    if (isDraggingLocalVol)
                    {
                        if (IsMouseButtonDown(MOUSE_LEFT_BUTTON))
                        {
                            ...
                            float newRatio = (newHandleY - vTrack.y) / (vTrack.height - vHandleH);
                            localVolScroll = -(newRatio * (volContentHeight - fixedListHeight));
                        }
                        else
                        {
                    ...
                    DrawRectangleRounded(vHandle, 1.0f, 10, isHoveredVScroll ? GRAY : Fade(DARKGRAY, 0.7f));
                    EndScissorMode();
                    BeginScissorMode((int)panelArea.x, (int)panelArea.y, (int)panelArea.width, (int)panelArea.height);
                }
            }
    ...
    Rectangle btnSave = { panelArea.x + (panelArea.width - btnWidth) / 2.0f, panelArea.y + masterScrollY + abs_btnSaveY, btnWidth, btnHeight };
    if (btnSave.y + btnSave.height >= panelArea.y && btnSave.y <= panelArea.y + panelArea.height)
    {
        ...
        if (forceShowAllErrors || showVolError)
        {
            ...
            float errFontSize = 20.0f;
            DrawTextEx(_Font[0], errorMsg, (Vector2){ panelArea.x + (panelArea.width - MeasureTextEx(_Font[0], errorMsg, errFontSize, 1).x) / 2.0f, btnSave.y + btnSave.height + 10.0f }, errFontSize, 1, RED);
        }
    }
    ...
    if (totalContentHeight > panelArea.height)
    {
        ...
        if (isDraggingMaster)
        {
            if (IsMouseButtonDown(MOUSE_LEFT_BUTTON))
            {
                ...
                float newRatio = (newHandleY - scrollTrack.y) / (scrollTrack.height - sHandleH);
                masterScrollY = -(newRatio * (totalContentHeight - panelArea.height));
                *volWheel = masterScrollY; 
            }
            else
            {
        ...
        DrawRectangleRounded(sHandle, 1.0f, 10, isHoveredScroll ? GRAY : Fade(DARKGRAY, 0.7f));
    }
    EndScissorMode();
    ...
    if (showAddVolPopup)
    {
        ...
        float popW = 450;
        float popH = 270;
        Rectangle popRec = { (GetScreenWidth() - popW)/2, (GetScreenHeight() - popH)/2, popW, popH };
        
        DrawRectangleRounded(popRec, 0.05f, 10, WHITE);
        DrawRectangleRoundedLinesEx(popRec, 0.05f, 10, 3, GRAY);

        DrawTextEx(_Font[0], "THÊM TẬP TRUYỆN MỚI", (Vector2){popRec.x + 80, popRec.y + 20}, 24, 1, MAROON);

        DrawTextEx(_Font[3], "Tên/Số tập (Nhập 0 nếu truyện lẻ):", (Vector2){popRec.x + 30, popRec.y + 75}, 18, 1, DARKGRAY);
        volNameIn.box = (Rectangle){ popRec.x + 325, popRec.y + 65, 90, 40 }; 
        
        DrawTextEx(_Font[3], "Số lượng nhập:", (Vector2){popRec.x + 30, popRec.y + 125}, 18, 1, DARKGRAY);
        volQtyIn.box = (Rectangle){ popRec.x + 180, popRec.y + 115, 235, 40 };
        ...
        Color nameLineCol = volNameIn.isFocused ? TEALBLUE : (highlightNameRed ? RED : GRAY);
        Color qtyLineCol = volQtyIn.isFocused ? TEALBLUE : (highlightQtyRed ? RED : GRAY);

        DrawRectangleRec(volNameIn.box, volNameIn.isFocused ? WHITESMOKE : LIGHTGRAY);
        DrawRectangleLinesEx(volNameIn.box, 2, nameLineCol);
        DrawRectangleRec(volQtyIn.box, volQtyIn.isFocused ? WHITESMOKE : LIGHTGRAY);
        DrawRectangleLinesEx(volQtyIn.box, 2, qtyLineCol);
        ...
        BeginScissorMode(volNameIn.box.x, volNameIn.box.y, volNameIn.box.width, volNameIn.box.height);
        DrawTextEx(_Font[3], volNameIn.text, (Vector2){volNameIn.box.x + 10, volNameIn.box.y + 10}, 18, 1, BLACK);
        if(volNameIn.isFocused && (int)(GetTime()*2)%2==0)
            DrawRectangle(volNameIn.box.x + 12 + MeasureTextEx(_Font[3], volNameIn.text, 18, 1).x, volNameIn.box.y + 5, 2, 30, BLACK);
        EndScissorMode();
        
        BeginScissorMode(volQtyIn.box.x, volQtyIn.box.y, volQtyIn.box.width, volQtyIn.box.height);
        DrawTextEx(_Font[3], volQtyIn.text, (Vector2){volQtyIn.box.x + 10, volQtyIn.box.y + 10}, 18, 1, BLACK);
        if(volQtyIn.isFocused && (int)(GetTime()*2)%2==0)
            DrawRectangle(volQtyIn.box.x + 12 + MeasureTextEx(_Font[3], volQtyIn.text, 18, 1).x, volQtyIn.box.y + 5, 2, 30, BLACK);
        EndScissorMode();
        
        if (showVolDuplicateError)
        {
            DrawTextEx(_Font[0], "* Tên tập truyện này đã tồn tại!", (Vector2){popRec.x + 115, popRec.y + 165}, 16, 1, RED);
        }

        Rectangle btnConf = { popRec.x + 70, popRec.y + 200, 120, 40 };
        Rectangle btnCanc = { popRec.x + 260, popRec.y + 200, 120, 40 };
        bool hConf = CheckCollisionPointRec(size.Mouse, btnConf);
        bool hCanc = CheckCollisionPointRec(size.Mouse, btnCanc);

        DrawRectangleRounded(btnConf, 0.2f, 10, hConf ? LIME : GREEN);
        DrawTextEx(_Font[3], "Xác nhận", (Vector2){btnConf.x + 20, btnConf.y + 10}, 18, 1, WHITE);
        
        DrawRectangleRounded(btnCanc, 0.2f, 10, hCanc ? MAROON : RED);
        DrawTextEx(_Font[3], "Hủy bỏ", (Vector2){btnCanc.x + 30, btnCanc.y + 10}, 18, 1, WHITE);
...
int ShowBooks_Panel(Size size, ManageBooksUI UI, float *wheel, Font *_Font, BookList *Books, int filterMode, const char *filterValue, Role _role, bool *showPagination, int *requestDeleteIndex, bool showOnlyInStock)
{
    ...
    if (distanceBooks < 20.0f) distanceBooks = 20.0f;
    if (distanceBooks > 60.0f) distanceBooks = 60.0f;

    Rectangle hitbox = { UI.Panel.x + UI.Panel.width * 0.05f, *wheel + UI.Panel.y + UI.Panel.height * 0.03f, UI.Panel.width * 0.9f, UI.Panel.height * 0.08f * (pow(0.3, size.Scale.x) + 0.6) };
    ...
    for (int i = 0; i < Books->count && displayCount < Books->QuantityForOnePage; i++)
    {
        ...
        if (filterMode == 1)
        {
            if (filterValue != NULL && strcmp(filterValue, Books->theArray[i].AuthorBook) == 0)
                flag = 1;
        } 
        else if (filterMode == 2)
        {
            if (filterValue != NULL && strcmp(filterValue, Books->theArray[i].TypeBook) == 0)
                flag = 1;
        } 
        else if (filterMode == 3)
        {
            if (filterValue != NULL && strcmp(filterValue, Books->theArray[i].PublisherBook) == 0)
                flag = 1;
        } 
    ...
    for (int idx = 0; idx < displayCount; idx++)
    {
        ...
        int PriceFontSize = hitbox.height * 0.35f; 
        ...
        float stockFontSize = PriceFontSize * 0.9f;
        float stockW = MeasureTextEx(_Font[0], stockStr, stockFontSize, 1).x;
        float separatorW = MeasureTextEx(_Font[0], " | ", PriceFontSize, 1).x;
        
        float badgePadding = 8.0f;
        float badgeW = stockW + badgePadding * 2;
        float badgeH = stockFontSize * 1.4f;
        float totalW = priceW + separatorW + badgeW;

        float startX = rightAnchor - totalW;
        float textY = hitbox.y + (hitbox.height - PriceFontSize) / 2;

        DrawTextEx(_Font[0], priceStr, (Vector2){startX, textY}, PriceFontSize, 1, GOLDACCENT);
        DrawTextEx(_Font[0], " | ", (Vector2){startX + priceW, textY}, PriceFontSize, 1, GRAY);

        Rectangle badgeRec = { startX + priceW + separatorW, hitbox.y + (hitbox.height - badgeH)/2, badgeW, badgeH };
        Color badgeBg = Books->theArray[i].TotalStock > 0 ? Fade(GREEN, 0.2f) : Fade(RED, 0.2f);
        Color badgeBorder = Books->theArray[i].TotalStock > 0 ? LIME : RED;
        
        DrawRectangleRounded(badgeRec, 0.3f, 10, badgeBg);
        DrawRectangleRoundedLinesEx(badgeRec, 0.3f, 10, 1.5f, badgeBorder);
        DrawTextEx(_Font[0], stockStr, (Vector2){badgeRec.x + badgePadding, badgeRec.y + (badgeRec.height - stockFontSize)/2}, stockFontSize, 1, WHITE);

        rightAnchor = startX - padding;
...
void ShowAuthor_Panel(Size size, ManageBooksUI UI, float *wheel, Font *_Font, AuthorList *Authors, char *selectedAuthor, BookList *Books)
{
    ...
    for (int i = 0; i < displayCount; i++)
    {
        ...
        DrawTextEx(_Font[0], Authors->Author[i].code, (Vector2){hitbox.x + 20, hitbox.y + (itemHeight - nameFont)/2}, nameFont, 1, isHovered ? GOLDACCENT : LIGHTGRAY);
        ...
        DrawTextEx(_Font[0], countStr, (Vector2){hitbox.x + hitbox.width - countW - 20, hitbox.y + (itemHeight - countFont)/2}, countFont, 1, isHovered ? WHITE : GRAY);
...
void ShowPublisher_Panel(Size size, ManageBooksUI UI, float *wheel, Font *_Font, PublisherList *Publishers, char *selectedPublisher, BookList *Books)
{
    ...
    for (int i = 0; i < displayCount; i++)
    {
        ...
        DrawRectangleRounded(hitbox, 0.2f, 20, isHovered ? Fade(TEALBLUE, 0.8f) : Fade(BLACK, 0.7f));
        int nameFont = (int)(itemHeight * 0.5f);
        DrawTextEx(_Font[0], Publishers->Publisher[i], (Vector2){hitbox.x + 30, hitbox.y + (itemHeight - nameFont)/2}, nameFont, 1, isHovered ? GOLDACCENT : WHITE);
...
void ShowType_Panel(Size size, ManageBooksUI UI, float *wheel, Font *_Font, Type *Types, char *selectedType, BookList *Books)
{
    ...
    for (int i = 0; i < displayCount; i++)
    {
        ...
        DrawRectangleRounded(hitbox, 0.2f, 20, isHovered ? Fade(TEALBLUE, 0.8f) : Fade(BLACK, 0.7f));
        int nameFont = (int)(itemHeight * 0.5f);
        DrawTextEx(_Font[0], Types->Type[i], (Vector2){hitbox.x + 30, hitbox.y + (itemHeight - nameFont)/2}, nameFont, 1, isHovered ? GOLDACCENT : WHITE);
...
bool ContextualBookSearch(Book book, const char *query)
{
    ...
    if (strstr(normCombined, normQuery) != NULL)
        return true;
...
bool ManageBooksFunc(MANAGEBOOKS_STATE *State, InputBox *FindBar, Texture2D Icon_Find, ManageBooksUI UI, Size size, Font *_Font)
{
    ...
    Vector2 TextWidth_Home = MeasureTextEx(_Font[0], "Home", 24, 2);
    Rectangle HomeBox = { UI.TitleBox.width * 0.03f, UI.TitleBox.height * 0.9f - TextWidth_Home.y, TextWidth_Home.x, TextWidth_Home.y };

    if (CheckCollisionPointRec(size.Mouse, HomeBox))
    {
        DrawTextEx(_Font[0], "Home", (Vector2) { HomeBox.x - 8, HomeBox.y - 4 }, 26, 3, GOLDACCENT);
        DrawLineEx((Vector2) { HomeBox.x, HomeBox.y + HomeBox.height }, (Vector2) { HomeBox.x + HomeBox.width, HomeBox.y + HomeBox.height }, 3, BLACK);
        if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
        {
            *State = MANAGEBOOKS_Dashboard;
            _return = 1;
    ...
    {
        DrawTextEx(_Font[0], "Home", (Vector2) { HomeBox.x, HomeBox.y }, 24, 2, BLACK);
        DrawLineEx((Vector2) { HomeBox.x, HomeBox.y + HomeBox.height }, (Vector2) { HomeBox.x + HomeBox.width, HomeBox.y + HomeBox.height }, 3, BLACK);
    }
    ...
    Rectangle AuthorBox = { HomeBox.x + HomeBox.width + UI.TitleBox.width * 0.04f, HomeBox.y, TextWidth_F.x, TextWidth_F.y };
