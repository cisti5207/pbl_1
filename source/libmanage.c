...
Font SetFontUTF8(const char *_font, int _fontSize)
{
    int codepoints[] = {
        ...
        258, 259, 272, 273, 296, 297, 360, 361, 416, 417, 431, 432,
...
void DrawTextAutoWrap(Font font, const char *text, Rectangle bounds, float fontSize, float spacing, Color color)
{
    ...
    float lineHeight = fontSize + 2.0f; // Khoảng cách giữa các dòng
    ...
    while (*ptr != '\0')
    {
        ...
        // 2. Xử lý dấu cách (tự động rớt dòng nếu người dùng gõ quá nhiều dấu cách)
        if (*ptr == ' ')
        {
            if (currentX + spaceWidth > bounds.x + bounds.width)
            {
                currentX = bounds.x;
                currentY += lineHeight;
            }
            else if (currentX > bounds.x)
            {
                currentX += spaceWidth;
            }
            ptr++;
        ...
        // 5. Kiểm tra tràn lề dưới (Cắt bớt nếu hết chỗ)
        ...
        // 6. Vẽ từ hiện tại
        DrawTextEx(font, wordBuffer, (Vector2){currentX, currentY}, fontSize, spacing, color);

        // 7. Tịnh tiến tọa độ X
        currentX += wordSize.x;
        ptr += idx;
    }
}
...
bool StringContains(const char *haystack, const char *needle)
{
    ...
    for (int i = 0; i <= lenH - lenN; i++)
    {
        ...
        for (j = 0; j < lenN; j++)
        {
            ...
            if (c1 != c2)
                break; // Lệch 1 ký tự là bỏ qua ngay
        }
        if (j == lenN)
            return true; // Khớp toàn bộ needle
    }
    return false;
}
