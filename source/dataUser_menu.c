    ...
    void setupConsole() {
        system("mode con: cols=237 lines=72");
    ...
    User *TaoUser() {
        ...
        while(getchar()!= '\n');
        printf("Nhập số tiền cọc: ");
        scanf("%d", &NewNode->tiencoc);
         while(getchar()!= '\n');
        NewNode->next = NULL;
        return NewNode;
    }
...
int layGiaTuFile(char *maCanTim) {
    ...
    while (fgets(line, sizeof(line), f)) {
        char ma[20], ten[100], tenN[100], sl[10], tt[20], giaStr[30];
        
        // sscanf này sẽ bốc đúng 6 cột dựa trên dấu |
        if (sscanf(line, " %[^|] | %[^|] | %[^|] | %[^|] | %[^|] | %[^\n\r]", 
                   ma, ten, tenN, sl, tt, giaStr) >= 6) {
            
            trim(ma); 
            
            if (strcmp(ma, maCanTim) == 0) {
                fclose(f);
                
                char chiLaySo[20] = "";
                int j = 0;
                for(int i = 0; giaStr[i] != '\0'; i++) {
                    if(giaStr[i] >= '0' && giaStr[i] <= '9') {
                        chiLaySo[j++] = giaStr[i];
                    }
                }
                chiLaySo[j] = '\0';
                
                int giaCuoi = atoi(chiLaySo);
                return (giaCuoi > 0) ? giaCuoi : 3000;
            }
        }
    }
    fclose(f);
...
//if > số ngày mượn từ 1 ngày trở lên thì tính thêm phí 120% của 1 ngày
//if < số ngày mượn từ 1 ngày trở lên thì tính trả 50% số tiền cọc
    void TinhDoanhThu(char ngaycantinh[], User *head) {
        int n;
        User *p = head;
        while(p!=NULL) {
            if(p->trangthai == 1) {
                int giathue = layGiaTuFile(p->maphieuthue);
                n = tinhKhoangCach(p);
                if(n < p->songaythue) {
                    doanhthu += n*p->soluongthue*giathue + p->tiencoc*1/2;
                }
                else if(n==p->songaythue) {
                    doanhthu+=n*p->soluongthue*giathue;
                }
                else { 
                    doanhthu+= p->songaythue * p->soluongthue * giathue + (n-p->songaythue)*giathue*1.2;
                }
            }
        }
    }
...
void gotoxy(int x, int y) {
    ...
    void indanhsachnguoichuatra(User *head) {
        printf("╔═════════╦══════════════════════════════════════╦═══════════╦═══════════════════════════════════════╦══════════╦═══════════════╦══════════════╦══════════════╦══════════════╗\n");
        printf("║ Mã User ║            Tên người thuê            ║ Mã truyện ║            Tên truyện                 ║ Số lượng ║   Ngày thuê   ║ Số ngày thuê ║   Tiền cọc   ║  Trạng thái  ║\n");
        printf("║═════════║══════════════════════════════════════║═══════════║═══════════════════════════════════════║══════════║═══════════════║══════════════║══════════════║══════════════║\n");
        int row = whereY() + 1;
        User *p = head;
        int count = 0;
        while(p!=NULL) {
            if(p->trangthai == 0) {
                gotoxy(0, row); printf("║");
                gotoxy(2, row); printf("%s", p-> maphieuthue);
                gotoxy(10, row); printf("║");
                gotoxy(12, row); printf("%s", p->realname);
                gotoxy(49, row); printf("║");
                gotoxy(51, row); printf("%s", p->matruyen);
                gotoxy(61, row); printf("║");
                gotoxy(63, row); printf("%s", p->tentruyenreal);
                gotoxy(101, row); printf("║");
                gotoxy(105, row); printf("%d", p->soluongthue);
                gotoxy(112, row); printf("║");
                gotoxy(114, row); printf("%s", p->ngaythue);
                gotoxy(128, row); printf("║");
                gotoxy(134, row); printf("%d", p->songaythue);
                gotoxy(143, row); printf("║");
                gotoxy(145, row); printf("%dVND", p->tiencoc);
                gotoxy(158, row); printf("║");
                gotoxy(160, row); printf("Chưa trả");
                gotoxy(173, row); printf("║");
                row++;
                count++;
            } 
            p=p->next;
        ...
        printf("╚═════════╩══════════════════════════════════════╩═══════════╩═══════════════════════════════════════╩══════════╩═══════════════╩══════════════╩══════════════╩══════════════╝");
        printf("\n");
        if(count == 0) printf("Danh sách trống");
        else {
            printf("Số lượng người chưa trả: %d", count);
        }
        }
int main() {
User *head = NULL;
...
while(1) {
printf("\n --Quản lý thuê truyện-- \n");
printf("1. Thêm phiếu thuê mới.\n");
printf("2. Danh sách người chưa trả.\n");
printf("3. Xử lý trả sách.\n");
printf("4. Tính doanh thu trong ngày mình nhập.\n");
printf("5. Thoát.\n");
...
switch (choice) {
    ...
    case 2:
    system("cls");
    indanhsachnguoichuatra(head);
    break;
    case 3: 
    xulytrasach(head);
    break;
    case 4: 
    printf("Nhập ngày cần tính doanh thu: ");
    char date[15];
    scanf("%s", &date);
    TinhDoanhThu(date, head);
    case 0:
    return 0;
    default:
